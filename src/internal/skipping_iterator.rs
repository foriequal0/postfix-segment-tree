...
impl SkippingIterator {
    ...
    pub(crate) fn new_after_index(index: usize, end: usize) -> Self {
        let index = get_pivot(index, end);
        Self { index, end }
    }
...
// TODO: PROOF let pivot = get_pivot_index(index, end), index >= min_reachable_index_for_elements(pivot), pivot >= index
fn get_pivot(index: usize, end: usize) -> usize {
    ...
    while i < index {
        ...
        let node_id = step_maximally_skipping_node_id(end, leaf_node_id).unwrap();
        i = node_id.index() + 1;
    }
...
impl Iterator for SkippingIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(node_id) = step_maximally_skipping_node_id(self.end, leaf_node_id) {
...
impl Iterator for IncreasingSkippingIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let leaf_node_id = LeafNodeId::new(self.index);
        if let Some(node_id) = step_minimally_skipping_node_id(self.end, leaf_node_id) {
            self.index = node_id.index() + 1;
            Some(node_id)
        } else {
            None
...
fn step_maximally_skipping_node_id(elements: usize, id: LeafNodeId) -> Option<NodeId> {
...
fn max_reachable_elements_for_current_index(index: usize) -> usize {
    ...
    // TODO: proof
    index + (1 << index.trailing_zeros() as usize)
}

fn step_minimally_skipping_node_id(elements: usize, id: LeafNodeId) -> Option<NodeId> {
...
mod tests {
    ...
    fn test_skipping_iterator() {
        ...
        assert_eq!(iter(1), vec![id(0, 0)]);
        assert_eq!(iter(2), vec![id(1, 1)]);
        assert_eq!(iter(3), vec![id(1, 1), id(2, 0)]);
        assert_eq!(iter(4), vec![id(3, 2)]);
        assert_eq!(iter(5), vec![id(3, 2), id(4, 0)]);
        assert_eq!(iter(6), vec![id(3, 2), id(5, 1)]);
        assert_eq!(iter(7), vec![id(3, 2), id(5, 1), id(6, 0)]);
        assert_eq!(iter(8), vec![id(7, 3)]);
    }
    ...
    fn test_increasing_skip_indexing_iterator() {
        ...
        assert_eq!(iter(0, 8), vec![id(7, 3)]);
        assert_eq!(iter(1, 8), vec![id(1, 0), id(3, 1), id(7, 2)]);
        assert_eq!(iter(2, 8), vec![id(3, 1), id(7, 2)]);
        assert_eq!(iter(3, 8), vec![id(3, 0), id(7, 2)]);
        assert_eq!(iter(4, 8), vec![id(7, 2)]);
        assert_eq!(iter(5, 8), vec![id(5, 0), id(7, 1)]);
        assert_eq!(iter(6, 8), vec![id(7, 1)]);
        assert_eq!(iter(7, 8), vec![id(7, 0)]);
    ...
    fn test_increasing_skipping_iterator_levels_monotonically_increasing() {
        ...
        for elements in 0..N {
            ...
            for i in min_index..N {
                let mut iter = IncreasingSkippingIterator::new(min_index, elements);
    ...
    fn test_combined_iterator() {
        fn iter(index: usize, end: usize) -> (Vec<NodeId>, Vec<NodeId>) {
            let iter = SkippingIterator::new_after_index(index, end);
            let pivot = iter.index();
