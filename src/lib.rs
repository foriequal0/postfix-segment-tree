    ...
    for<'a> T: AddAssign<&'a T> + Default,
{
    ...
    pub fn sum(&self, index: usize, len: usize) -> T {
        ...
        let mut sum = T::default();
        let iter = SkippingIterator::new_after_index(index, index + len);
    ...
    for<'a> T: AddAssign<&'a T> + Default,
{
    ...
    pub fn update(&mut self, index: usize, element: T) {
        ...
        *self.get_leaf_node_mut(id) = element; // DIRTY: parents of `index`
        self.recalculate_nodes_after_update(id); // CLEAN: parents of `index`
    }
    ...
    pub fn push(&mut self, element: T) {
        let inserted_at = self.len();

        self.resize_by_one(); // DIRTY: parents of `self.len() - 1` after the operation, which is `inserted_at`

        let id = LeafNodeId::new(inserted_at);
        *self.get_leaf_node_mut(id) = element; // DIRTY: parents of `inserted_at`
        self.recalculate_nodes_after_update(id); // CLEAN: parents of `inserted_at
    }
    ...
    pub fn insert(&mut self, index: usize, element: T) {
        ...
        self.resize_by_one(); // DIRTY: parents of `self.len() - 1` after the operation
        self.shift_nodes_right_by_one(index); // DIRTY: all parents of `>= index`, which includes `self.len() - 1`

        let id = LeafNodeId::new(index);
        *self.get_leaf_node_mut(id) = element; // DIRTY: parents of `index`, which is included by all parents of `>= index`
        self.recalculate_nodes_after_bulk_update(id); // CLEAN: all parents of `>= index`
    }
    ...
    pub fn remove(&mut self, index: usize) {
        ...
        self.shift_nodes_left_by_one(index); // DIRTY: all parents of `>= index`
        self.truncate_by_one(); // CLEAN: parents of `self.len()` after the operation
